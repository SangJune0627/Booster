...
public class InputCombinations {
    ...
    private LinkedHashMap<Integer,List<Input>> inputPoolsEqualityCheck(LinkedHashMap<Integer, List<Input>> inputPools) {
...
//                            writeFile("EqualityCheck_"+(inputNum),packageAndImport+clazz.toString());
                        errorMessage = compileWholeClassFile("EqualityCheck_"+(inputNum),packageAndImport + clazz.toString());
                        if(!checkCompileError(errorMessage)){
                            ...
                            equalityCheckClass = ClassLoader.getSystemClassLoader().loadClass(packageNamePlusFileName("EqualityCheck_"+(inputNum)));
                        ...
                        System.out.println(String.format("Load running class : %s failed.", packageNamePlusFileName("EqualityCheck_"+(inputNum))));
                    } catch (InstantiationException | InvocationTargetException | IllegalAccessException e) {
                        System.out.println(String.format("Instantiate class : %s failed.", packageNamePlusFileName("EqualityCheck_"+(inputNum))));
                        e.printStackTrace();
                    } catch (Exception e){
                        e.printStackTrace();
                    }
...
//                            writeFile("EqualityCheck_"+(inputNum),packageAndImport+clazz.toString());
    ...
    private Set<MUTInput> processMUTs() {
...
//                        inputs = inputPoolEqualityCheck(inputs);
    ...
    private List<Input> processInputPool(CtTypeReference type, boolean isReceiverObject){
        ...
        HashMap<Pair<CtTypeReference, CtElement>, Set<List<CtElement>>> pairsToInputMap = CandidatePool.getVartypeToInputPool();
        ...
        if (type.isPrimitive()) {
//            if (containsKeyInDirectValuesMap) { //if there exists direct value input, we just use this for primitives
//                Set<List<CtElement>> converted = convertPrimitiveStructure(directValuesMap.get(type));
//                inputs = fillInputStructure(converted, type);
//            } else { //if there exists no direct value input for primitves, we use ones in pairsToInputMap
...
//                    }
//                } else {
//                    System.err.println("WARNING InputCombinations.processMUTs: no key exists in both direct value and Pair for primitive type " + type.toString());
//                }
//            }
            ...
            if (containsKeyInDirectValuesMap) { //if there exists direct value input, we additionally use this for input
                Set<List<CtElement>> converted = convertPrimitiveStructure(directValuesMap.get(type));
                inputs = fillInputStructure(converted, type);
//                System.out.println(directValuesMap.get(type));
            ...
            if (!containsKeyInDirectValuesMap && !containsKeyInPairsMap) {
                ...
                if (pairs != null) { // For some reason, there are few types that are different in MUTnameToArgtypes and varTypeNamePool
                    for (Pair<CtTypeReference, CtElement> pair : pairs) {
                        Set<List<CtElement>> inputSet = pairsToInputMap.get(pair);
                        inputs = fillInputStructure(inputSet, type, pair.getValue());
                    }
                } else {
                    System.err.println("WARNING InputCombinations.processMUTs: no key exists in both direct value and Pair for type " + type.toString());
                }
            }
        }
    ...
    private Set<Pair<CtTypeReference, CtElement>> furtherFindInPairs(HashMap<CtTypeReference, Set<Pair<CtTypeReference, CtElement>>> pairsMap, CtTypeReference type) {
    ...
    private List<Input> fillInputStructure(Set<List<CtElement>> inputs, CtTypeReference type) {
        ...
        for (List<CtElement> input : inputs) {
            ...
            if (input.size() == 1) {
                ...
                System.err.println("InputCombination.fillInputStructure: for directValue, input size should be 1. Type: " + type + ", Input: " + input);
                continue;
            }
...
//        ((CtCodeSnippetStatement) nullCheckStatement).setValue(String.format("System.out.println(object1().equals(((%s) (null))))", o1.getType().getQualifiedName().replace("$",".")));
//        reloads.add(nullCheckStatement);
//
//        CtStatement nullCheckStatement2 = facotry.Core().createCodeSnippetStatement();
//        ((CtCodeSnippetStatement) nullCheckStatement2).setValue("System.out.println(object1().equals(object2()))");
//        reloads.add(nullCheckStatement2);
//
//        CtStatement nullCheckStatement3 = facotry.Core().createCodeSnippetStatement();
//        ((CtCodeSnippetStatement) nullCheckStatement3).setValue("System.out.println(object1())");
//        reloads.add(nullCheckStatement3);
//
//        CtStatement nullCheckStatement4 = facotry.Core().createCodeSnippetStatement();
//        ((CtCodeSnippetStatement) nullCheckStatement4).setValue("System.out.println(object2())");
//        reloads.add(nullCheckStatement4);

        CtStatement ctStatement = facotry.Core().createCodeSnippetStatement();
        ((CtCodeSnippetStatement) ctStatement).setValue("return object1() == null ? object2() == null : object1().equals(object2())");
