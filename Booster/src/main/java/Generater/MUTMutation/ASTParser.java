...
//            System.out.println("Constructor: " + lastStatement);
                List<CtConstructorCall> construct = lastStatement.getElements(new TypeFilter<>(CtConstructorCall.class));
                mut = construct.get(0);
                lastMethodInvoke = true;
                ...
                if (isInvocation(lastTryStmt)) {
                    ...
                    List<CtConstructorCall> construct = lastTryStmt.getElements(new TypeFilter<>(CtConstructorCall.class));
                    mut = construct.get(0);
                } else {
                    System.err.println("WARN: No invocation or constructor call in last block of " + method.getSimpleName() + ": " + lastStatement);
                }
                lastMethodInvoke = true;
            } else {
                System.err.println("WARN: Last statement of " + method.getSimpleName() + " is neither invocation nor constructor: " + lastStatement);
                if (--stmtIndex >= 0)
     ...
     * @throws Exception
     */
    private static void processArguments(CtAbstractInvocation finalStmt, List<CtStatement> statements) throws Exception {
...
//            System.out.println(arg.toString());
    ...
    private static List<CtElement> getStatements(CtElement arg, List<CtStatement> statements) {
        ...
        for (CtStatement statement : statements) {
            if (arg.getPosition().getLine() == statement.getPosition().getLine() || arg.getPosition().getLine() > statement.getPosition().getLine())
    ...
    private static void processMUT(CtAbstractInvocation mut) throws Exception {
        ...
        if (mut instanceof CtInvocationImpl) {
            ...
            if (invoke.getTarget() instanceof CtVariableReadImpl) {
                ...
                return;
//                throw new Exception("unkonwn variable reference during ASTParser.processMUT for Invocation: " + mut.getExecutable().getSignature());
            }
                ...
                if(castType==null){
                    if(type.isPrimitive() || type.toString().equals("java.lang.String")){
                        if(arg.getElements(new TypeFilter<>(CtFieldRead.class)).size() == 1){
                            CandidatePool.insertVarTypeToInputPool(new Pair<CtTypeReference, CtElement>(type, arg), stats.subList(0, stats.size()-1));
                        } else {
                    ...
                    if(castType.isPrimitive() || castType.toString().equals("java.lang.String")){
                        if(arg.getElements(new TypeFilter<>(CtFieldRead.class)).size() == 1){ // (double) object.zero
                            CandidatePool.insertVarTypeToInputPool(new Pair<CtTypeReference, CtElement>(castType, arg), stats.subList(0, stats.size()-1));
                        } else {
                            if (arg.getElements(new TypeFilter<>(CtVariableReference.class)).size() == 0) { // (type) null
                                CandidatePool.insertDirectToValues(castType, arg);
                            } else if (arg.getElements(new TypeFilter<>(CtVariableReference.class)).size() == 1) { //we handle typecasted variable reference as a param such as numericEntityUnescaper0_10.translate(((java.lang.CharSequence) (charBuffer0_3)));
                                CandidatePool.insertVarTypeToInputPool(new Pair<CtTypeReference, CtElement>(castType, arg), stats.subList(0, stats.size()-1));
                            } else {
...
//            System.out.println(ctMethod.toString());
    ...
    private static void processStatement(CtMethod testcase) {
        ...
        for (CtAbstractInvocation invoke : invokes) {
            ...
            for (CtStatement statement : statements) {
                try {
                    if (statement.getPosition().getLine() <= invoke.getPosition().getLine()) //|| (statement.getPosition().getLine() == invoke.getPosition().getLine()))
